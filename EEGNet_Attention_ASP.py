...
class AvgStatPooling(nn.Module):
    def __init__(self):
        super(AvgStatPooling, self).__init__()
...
class EEGNet_Attention_ASP(torch.nn.Module):
    """EEGNet with self attention."""
    ...
    def __init__(
        ...
        # adjust cnn_setpemporal_point_kernels to be the nearest number that is divisble by nhead
        if nhead % cnn_septemporal_point_kernels != 0:
            cnn_septemporal_point_kernels = round(cnn_septemporal_point_kernels / nhead) * nhead
        ...
        # Adding Attention to EEGNet
        self.attention = sb.nnet.attention.MultiheadAttention(nhead=nhead,d_model=cnn_septemporal_point_kernels,dropout=dropout)
        
        # Average statistical pooling module
        self.avg_stat_pooling = AvgStatPooling()
        
        # Layer norm
        self.layerNorm = sb.nnet.normalization.LayerNorm(cnn_septemporal_point_kernels)
        ...
        self.dense_module.add_module(
            ...
            sb.nnet.linear.Linear(
                input_size=cnn_septemporal_point_kernels*2,
    ...
    def forward(self, x):
        ...
        x = self.layerNorm(x + attn_x)
        
        # Reshape the self-attended features
        x = x.view(batch_size, num_channels, time_steps, num_channels_out)
        
        # Pass through ASP
        x = self.avg_stat_pooling(x)
